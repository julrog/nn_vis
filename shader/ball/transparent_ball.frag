...
void main()
{
    ...
    float add_color = 1.0;
    float sphere_density = clamp((normal.z + 0.2) / 1.2, 0.0, 1.0);
    ...
    float overall_opacity = 0.8 * pow(sphere_density, 1.5);
    float depth_opacity = 0.8;

    frag_color = vec4(frag_color.x, frag_color.y, frag_color.z,(base_opacity + depth * depth_opacity) * overall_opacity);
