...
class EdgeRenderer(Renderer):
    def __init__(self, edge_processor: EdgeProcessor, grid: Grid):
        ...
        self.render_funcs['sample_ellipsoid_transparent'] = generate_render_function(OGLRenderFunction.ARRAYS_INSTANCED,
                                                                                     GL_POINTS, add_blending=True)
        self.element_count_funcs['sample_point'] = generate_element_count_func(
            edge_processor)
        self.element_count_funcs['sample_line'] = generate_element_count_func(
            edge_processor)
        self.element_count_funcs['sample_sphere'] = generate_element_count_func(
            edge_processor)
        self.element_count_funcs['sample_transparent_sphere'] = generate_element_count_func(
            edge_processor)
        self.element_count_funcs['sample_ellipsoid_transparent'] = generate_element_count_func(
            edge_processor)
    ...
    def render(self, set_name: str, cam: BaseCamera, config: RenderingConfig = None, show_class: int = 0):
        ...
        if isinstance(current_set, LayeredRenderSet):
            current_set.set_buffer_divisor(
                [(0, 1), (1, self.edge_processor.max_sample_points)])
        near: float = 0.0
        far: float = 0.0
        if set_name == 'sample_ellipsoid_transparent' or set_name == 'sample_transparent_sphere':
        ...
        current_set.set_uniform_data([('projection', cam.projection, 'mat4'),
                                      ...
                                      ('object_radius',
                                       self.edge_processor.sample_length * 0.5, 'float'),
                                      ('importance_threshold',
                                       self.importance_threshold, 'float'),
                                      ('importance_max',
                                       self.edge_processor.edge_max_importance, 'float'),
                                      ('max_sample_points',
                                       self.edge_processor.max_sample_points, 'int'),
