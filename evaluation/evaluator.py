...
class ImportanceEvaluator:
    def __init__(self, model_data: ModelData):
        ...
        self.importance_type: ImportanceType = ImportanceType(model_data.get_importance_type())
    ...
    def get_importance(self, edge_alpha: float, classes_importance: List[float]) -> float:
        ...
        if self.relevant_classes is not None:
            ...
            edge_sum_class_importance = edge_sum_class_importance / float(len(self.relevant_classes))
        else:
        ...
        for layer_id, layer in enumerate(importance_data.edge_importance_data):
            ...
            for input_node_id, input_node in enumerate(layer):
                for edge_id, edge_alpha in enumerate(input_node):
                    ...
                    edge_importance: float = self.get_importance(edge_alpha, classes_importance)
                    if edge_importance <= importance_threshold:
                        pruned_edges += 1
                        layer_weights[0][input_node_id][edge_id] = 0.0
                    else:
                        remaining_edges += 1
            self.model_data.model.layers[layer_id + 1].set_weights(layer_weights)
    ...
    def accuracy_report(self, truths: np.array, predictions: np.array) -> Dict[str, any]:
        ...
        for i in range(self.model_data.get_num_classes()):
            ...
            accuracy_report[str(i)] = (true_positive_rate + true_negative_rate) / 2.0
    ...
    def create_evaluation_data(self, step_size: int = 1, start_percentage: int = 0, end_percentage: int = 100):
        ...
        for prune_percentage in range(start_percentage, end_percentage + step_size, step_size):
            ...
            self.prune_model(str(prune_percentage), importance_data, importance_threshold)
