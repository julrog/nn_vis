...
class BaseRenderSet:
    def __init__(self, shader: BaseShader, render_func: Callable, element_count_func: Callable) -> None:
...
class RenderSet(BaseRenderSet):
    def __init__(self, shader: BaseShader, data_handler: VertexDataHandler, render_func: Callable,
                 element_count_func: Callable) -> None:
...
class OverflowingRenderSet(BaseRenderSet):
    ...
    def render_sub(self, buffer_index: int = 0) -> None:
        if self.shader is not None:
            self.shader.use()
