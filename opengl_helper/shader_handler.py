...
class RenderShaderHandler(metaclass=Singleton):
    ...
    def create(self, shader_setting: ShaderSetting) -> RenderShader:
        ...
        vertex_src: str = self.get_processed_src(os.path.join(self.shader_dir, shader_setting.vertex))
        fragment_src: str = self.get_processed_src(os.path.join(self.shader_dir, shader_setting.fragment))
        geometry_src: str or None = None
        if shader_setting.geometry is not None:
            geometry_src = self.get_processed_src(os.path.join(self.shader_dir, shader_setting.geometry))
        ...
        if '$$' in processed_line:
            for node_buffer_group in range(int(math.ceil((self.num_classes + (ADDITIONAL_NODE_BUFFER_DATA - 4)) / 4.0))):
                ...
                if '$r_nodebuffer_group_id$' in new_line:
                    new_line = new_line.replace('$r_nodebuffer_group_id$', str(node_buffer_group))
                    added = True
            ...
            for edge_buffer_group in range(int(math.ceil((self.num_classes * 2 + ADDITIONAL_EDGE_BUFFER_DATA) / 4))):
                ...
                if '$r_edgebuffer_group_id$' in new_line:
                    new_line = new_line.replace('$r_edgebuffer_group_id$', str(edge_buffer_group))
                    added = True
            ...
            for class_id in range(self.num_classes):
                ...
                if '$r_class_color$' in new_line:
                    new_line = new_line.replace('$r_class_color$', CLASS_COLOR[class_id])
                    added = True
