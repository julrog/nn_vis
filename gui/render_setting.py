...
class RenderSettings:
    def __init__(self, root: LabelFrame, name: str, change_setting_func: Callable, config: RenderingConfig,
        ...
        if shader_settings is not None:
            ...
            self.apply_settings.grid(
                row=len(self.shader_settings), column=0, columnspan=2)
            self.shader_setting_frame.grid(row=0, column=1, rowspan=len(
                self.render_radio_buttons), padx=1, pady=1)
