from tkinter import Button, Checkbutton, IntVar, Label, LabelFrame, Tk
...
def set_stat_frame(gui_root: Tk, settings: Dict[Any, Any]):
    ...
    for stat in range(len(STATISTIC_NAME)):
        settings[StatisticLink(stat)] = SettingField(
            stats_frame, STATISTIC_NAME[stat] + ':', row=stat, column=0)
...
def set_render_frame(gui_root: Tk, change_render_config: Callable, change_setting: Callable,
    ...
    render_frame: LabelFrame = LabelFrame(
        gui_root, text='Render Settings', width=60, padx=5, pady=5)
    render_frame.grid(row=0, column=3, columnspan=2, rowspan=3, padx=5, pady=5)

    RenderSettings(render_frame, 'Grid', change_render_config,
                   render_config, 'grid_render_mode', None, row=0, column=0)
    RenderSettings(render_frame, 'Edge', change_render_config, render_config, 'edge_render_mode', EDGE_SHADER_UNIFORM,
                   row=1, column=0)
    RenderSettings(render_frame, 'Node', change_render_config, render_config, 'node_render_mode', NODE_SHADER_UNIFORM,
                   row=2, column=0)

    class_setting_frame: LabelFrame = LabelFrame(
        render_frame, text='Class Visibility', width=60, padx=5, pady=5)
    ...
    return RadioButtons(class_setting_frame, show_class_names, class_show, command=change_setting, option='show',
...
def set_architecture_frame(gui_root: Tk, save_processed_nn_file: Callable,
    ...
    architecture_frame: LabelFrame = LabelFrame(
        gui_root, text='Neural Network Architecture', width=60, padx=5, pady=5)
    ...
    load_button: Button = Button(
        architecture_frame, text='Load Network', width=20, command=open_importance_file)
    load_button.grid(row=2, column=0, columnspan=3)
    generate_button: Button = Button(
        architecture_frame, text='Generate Network', width=20, command=generate)
    generate_button.grid(row=3, column=0, columnspan=3)
    add_layer_button: Button = Button(
        architecture_frame, text='Add Layer', command=add_layer)
    add_layer_button.grid(row=4, column=1)
    clear_layer_button: Button = Button(
        architecture_frame, text='Clear Layer', command=clear_layer)
...
def set_processing_frame(gui_root: Tk, processing_config: ProcessingConfig, change_setting: Callable,
    ...
    processing_frame: LabelFrame = LabelFrame(
        gui_root, text='Processing', width=60, padx=5, pady=5)
    processing_frame.grid(row=0, column=1, columnspan=2,
                          rowspan=3, padx=5, pady=5)

    action_frame: LabelFrame = LabelFrame(
        processing_frame, text='Actions', width=60, padx=5, pady=5)
    ...
    smoothing_checkbox: Checkbutton = Checkbutton(action_frame, text='Smoothing', variable=smoothing_status,
                                                  command=lambda: change_processing_config('smoothing',
                                                                                           smoothing_status.get()))
    change_setting('edge', 'smoothing', smoothing_status.get())
    smoothing_checkbox.grid(row=1, column=0)

    setting_frame: LabelFrame = LabelFrame(
        processing_frame, text='Settings', width=60, padx=5, pady=5)
    setting_frame.grid(row=0, column=0, padx=5, pady=5)
    processing_setting: ProcessingSetting = ProcessingSetting(
        processing_config, setting_frame)
