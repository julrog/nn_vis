from tkinter import END, Button, Entry, Label, LabelFrame
...
class LayerSettings:
    def __init__(self, root: LabelFrame, layer_id: int, row: int, column: int, remove_func: Callable):
        ...
        self.remove_button: Button = Button(
            root, text='Remove', command=lambda: remove_func(self.layer_id))
